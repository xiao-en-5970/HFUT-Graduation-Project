...
## 2026-05-02（求助独立：`GET /goods` 新增 `category` 过滤）

- **修改** `GET /api/v1/goods` 新增 Query 参数 **`category`**：
    - `category=1` 仅返回二手买卖
    - `category=2` 仅返回有偿求助
    - 缺省 / 其它值 = 不过滤（向后兼容）
    - **两条链路都过滤**：`sort=recommend` 的个性化推荐 与 普通 List 查询都会应用 category，避免"市集混进求助"/"
      求助混进商品"。
- **语义调整**：前端「市集」固定传 `category=1`；新增的「求助」底栏 tab 会传 `category=2`（配合 `articles.type=2` 文字提问混排）。
- **修改** 创建求助（`POST /api/v1/goods`，`goods_category=2`）：`goods_type` 若为非法值会回退到 **3 在线**（原为 1
  送货上门），因为求助通常无物流。前端在「求助」入口不再展示履约方式 / 地址 / 收款码字段。
...
- `GET .../orders/:id/messages` **不返回** 历史 `msg_type=3` 记录（旧数据仍存库，仅列表过滤）。
- 移除错误码 **2705**（原「官方通知账号未配置」）。
